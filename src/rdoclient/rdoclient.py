...
class RandomOrgClient:
    ...
    generate_signed_integers -- get a signed response containing a list of random integers and a signature.
    generate_signed_decimal_fractions -- get a signed response containing a list of random doubles and a signature.
    generate_signed_gaussians -- get a signed response containing a list of random numbers and a signature.
    generate_signed_strings -- get a signed response containing a list of random strings and a signature.
    generate_signed_UUIDs -- get a signed response containing a list of random UUIDs and a signature.
    generate_signed_blobs -- get a signed response containing a list of random blobs and a signature.
    ...
    create_integer_cache -- get a RandomOrgCache from which to obtain a list of random integers.
    create_decimal_fraction_cache -- get a RandomOrgCache from which to obtain a list of random doubles.
    create_gaussian_cache -- get a RandomOrgCache from which to obtain a list of random numbers.
    create_string_cache -- get a RandomOrgCache from which to obtain a list of random strings.
    create_UUID_cache -- get a RandomOrgCache from which to obtain a list of random UUIDs.
    create_blob_cache -- get a RandomOrgCache from which to obtain a list of random blobs.
    ...
    def __init__(self, api_key, 
                 blocking_timeout=24.0*60.0*60.0,
                 http_timeout=120.0,
                 serialized=True,
                ):
        """
        ...
        api_key -- API key obtained from the RANDOM.ORG website, see: https://api.random.org/api-keys
    ...
    def create_decimal_fraction_cache(self, 
                                      n, 
                                      decimal_places, 
                                      replacement=True, 
                                      cache_size=20,
        ...
        # get the request object for use in all requests from this cache
        request = self._generate_request(_DECIMAL_FRACTION_METHOD, params)
        return RandomOrgCache(self._send_request, self._extract_doubles, request, cache_size, bulk_n, n)
    def create_gaussian_cache(self, 
                              n, 
                              mean, 
                              standard_deviation,
                              significant_digits, 
    ...
    def get_requests_left(self):
        ...
        if self._requests_left is None or \
            time.perf_counter() > self._last_response_received_time + _ALLOWANCE_STATE_REFRESH_SECONDS:
            self._get_usage()
    ...
    def _send_request_core(self, request):
        ...
        wait = self._advisory_delay - (time.perf_counter() - self._last_response_received_time)
        ...
        if 'error' in data:
            ...
            if code in [-32700] + list(
                ...
                return {'exception': RuntimeError(f"Error {str(code)}: {message}")}
            # RandomOrgKeyNotRunningError, API key not running, from 
            # RANDOM.ORG Errors: https://api.random.org/json-rpc/1/error-codes
            elif code == 401:
                return {'exception': RandomOrgKeyNotRunningError(f"Error {str(code)}: {message}")}
            ...
            # Errors: https://api.random.org/json-rpc/1/error-codes
                ...
                self._backoff = datetime.utcnow().replace(day=datetime.utcnow().day+1, hour=0, minute=0, second=0, microsecond=0)
                self._backoff_error = f"Error {str(code)}: {message}"
                return {'exception': RandomOrgInsufficientRequestsError(self._backoff_error)}
            # RandomOrgInsufficientBitsError, bits allowance exceeded,
            # from RANDOM.ORG Errors:
            # https://api.random.org/json-rpc/1/error-codes
            elif code == 403:
                return {'exception': RandomOrgInsufficientBitsError(f"Error {str(code)}: {message}")}
            # ValueError, error codes listed under RANDOM.ORG Errors:
            # https://api.random.org/json-rpc/1/error-codes
            else:
                return {'exception': ValueError(f"Error {str(code)}: {message}")}
        # Update usage stats
    ...
    def _extract_signed_response(self, response, extract_function):
        ...
        """
        return {
                'data':extract_function(response), 
